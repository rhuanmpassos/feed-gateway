...
2. [Todas as Telas](#-todas-as-telas)
3. [Design Visual](#-design-visual)
4. [Conexão com o Gateway](#-conexão-com-o-gateway)
5. [Autenticação e Onboarding](#-fluxo-de-autenticação-e-onboarding)
6. [Feeds de Notícias](#-feeds-de-notícias)
7. [Ações do Card (Like, Bookmark, Share)](#-ações-do-card)
8. [Sistema de Interações](#-sistema-de-interações-obrigatório)
9. [Perfil e Padrões](#-perfil-e-padrões-do-usuário)
10. [WebSocket](#-websocket-tempo-real)
...
│   │  │                    MAIN APP                             │    │
│   │  │  ┌──────────┬──────────┬──────────┬──────────┐         │    │
│   │  │  │ Para Você│  Agora   │  Salvos  │  Perfil  │         │    │
│   │  │  └──────────┴──────────┴──────────┴──────────┘         │    │
│   │  └────────────────────────────────────────────────────────┘    │
│   │         │              │           │           │                │
│   │         ▼              ▼           ▼           ▼                │
│   │   ┌──────────┐   ┌──────────┐ ┌────────┐ ┌──────────┐          │
│   │   │  ARTIGO  │   │  ARTIGO  │ │ ARTIGO │ │PREFERÊNC.│          │
│   │   │ (WebView)│   │ (WebView)│ │(WebView│ │(Categorias│          │
│   │   └──────────┘   └──────────┘ └────────┘ └──────────┘          │
│   │                                                │                │
│   │                                                ▼                │
│   │                                          ┌──────────┐          │
...
### 1. Tela: Splash Screen

**Duração:** 2-3 segundos  
**Propósito:** Carregar assets, fonts e verificar autenticação
...
### 2. Tela: Onboarding - Proposta de Valor

**Propósito:** Apresentar o app e convencer o usuário
...
Fundo: #0A0A0B
Título: #FFFFFF, 28px, Playfair Display Bold
Subtítulo: #A1A1A6, 16px, Inter
Botão: bg #FF3B30, text #FFFFFF
Link: #007AFF
```

---

### 3. Tela: Onboarding - Seleção de Categorias

**Propósito:** Coletar preferências iniciais (mínimo 3)
...
### 4. Tela: Onboarding - Notificações (Opcional)

**Propósito:** Pedir permissão de push (pode pular)
...
Botão principal: bg #FF3B30, text #FFFFFF
Link secundário: #636366
```

**Após essa tela:** Vai direto para a Home (Para Você)

---

### 5. Tela: Home - Para Você (Tab Principal)

**Propósito:** Feed personalizado pelo algoritmo
...
│  │ URGENTE  │                          │
│  └──────────┘                          │
│  Congresso aprova reforma              │
│  tributária em votação...              │
│                                         │
│  G1 • 5 min                            │
│                                         │
│  ⭐        🔖        ↗️                 │
...
Header:
  - Título: 24px, Playfair Display Bold, #FFFFFF
  - Saudação: 16px, Inter, #A1A1A6
  - Badge "Feito para você": bg rgba(255,69,0,0.15), text #FF4500
...
Bottom Tab:
  - bg: #0A0A0B
  - altura: 80px (com safe area)
  - ícone ativo: #FFFFFF + dot
  - ícone inativo: #636366
```

**Interações:**
- Pull to refresh → atualiza feed
- Scroll down → carrega mais (infinite)
- Tap no card → abre artigo
- Tap nos botões → like/save/share

---

### 6. Tela: Agora (Tab Cronológico)

**Propósito:** Feed em ordem cronológica + Breaking News
...
Seção Breaking:
  ...
  - cards compactos (sem imagem grande)

Cards cronológicos:
  - Timestamp à esquerda: #636366, 12px
  - Thumbnail pequeno (60x60)
  - Layout horizontal
```

---

### 7. Tela: Salvos (Tab Bookmarks)

**Propósito:** Artigos salvos pelo usuário
...
│  ┌─────────────────────────────────┐   │
│  │ ┌─────┐                         │   │
│  │ │ IMG │ Título do artigo que    │   │
│  │ │60x60│ foi salvo pelo usuário  │   │
│  │ └─────┘                         │   │
│  │         G1 • Salvo há 2 dias    │   │
│  │                          🗑️     │   │
│  └─────────────────────────────────┘   │
...
Cards:
  ...
  - Thumbnail pequeno
  - Botão de remover (🗑️) à direita
  - Swipe left → confirmar remoção

Empty State (se não houver salvos):
  - Ícone: 🔖 grande
  - Texto: "Nenhum artigo salvo"
  - Subtexto: "Toque em 🔖 em qualquer artigo para salvá-lo aqui"
```
...
### 9. Tela: Editar Interesses

**Propósito:** Alterar preferências de categorias
...
│                                         │
│                                         │
│    ℹ️ Suas preferências afetam o feed   │
│       "Para Você"                       │
│                                         │
└─────────────────────────────────────────┘
...
Header:
  - Voltar: ← (tappable)
  - Salvar: #007AFF (só aparece se mudou algo)

Mesmo estilo de chips do onboarding
```

---

### 10. Tela: Artigo (WebView/Browser)

**Propósito:** Ler o artigo completo
...
Header:
  - Botão voltar: ←
  - Domínio do site: texto truncado
  - Progress bar: mostra % de scroll
  - Menu: ⋮ (abrir no browser, copiar link)

Footer fixo:
  - bg: #0A0A0B
  - altura: 60px
  - 3 botões de ação (mesmo estilo dos cards)
  - Safe area na parte inferior

Tracking:
  - Ao abrir: trackClick(articleId)
  - Ao fechar: trackView(articleId, duration)
```

---

### 11. Tela: Configurações de Notificação
...
| # | Tela | Tipo | Propósito |
|---|------|------|-----------|
| 1 | Splash | Stack | Carregamento inicial |
| 2 | Onboarding - Proposta | Stack | Apresentar o app |
| 3 | Onboarding - Categorias | Stack | Coletar preferências |
| 4 | Onboarding - Notificações | Stack | Pedir permissão push |
| 5 | Para Você | Tab | Feed personalizado |
| 6 | Agora | Tab | Feed cronológico |
| 7 | Salvos | Tab | Artigos bookmarkados |
| 8 | Perfil | Tab | Configurações |
| 9 | Editar Interesses | Modal/Stack | Alterar categorias |
| 10 | Artigo | Stack | Ler artigo (WebView) |
| 11 | Notificações | Stack | Config de push |
...
|--------|------|----------|
| 5.0.0 | 2025-12-11 | Documentação completa de todas as 11 telas do app (Splash → Perfil) |
